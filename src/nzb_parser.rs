...
/// Patterns: `setname.vol00+01.par2` and `setname.vol00-01.par2`.
///
/// Both separators are emitted by real PAR2 creators/indexers. The plus form
/// is the traditional par2cmdline spelling; the hyphen form is also common in
/// NZBs generated by newer posting tools.
pub fn parse_par2_filename(filename: &str) -> (Option<String>, Option<u32>, Option<u32>) {
    ...
    // Check for .volNN+NN / .volNN-NN pattern.
    if let Some(vol_pos) = without_ext.to_lowercase().rfind(".vol") {
        ...
        if let Some(separator_pos) = vol_part.find(['+', '-']) {
            let vol_text = &vol_part[..separator_pos];
            let blocks_text = &vol_part[separator_pos + 1..];
            if !vol_text.is_empty()
                && !blocks_text.is_empty()
                && vol_text.chars().all(|c| c.is_ascii_digit())
                && blocks_text.chars().all(|c| c.is_ascii_digit())
                && let (Ok(vol), Ok(blocks)) = (vol_text.parse(), blocks_text.parse())
            {
...
mod tests {
    ...
    #[test]
    fn test_parse_par2_with_hyphenated_volume() {
        let (set, vol, blocks) = parse_par2_filename("data.vol63-67.par2");
        assert_eq!(set, Some("data".into()));
        assert_eq!(vol, Some(63));
        assert_eq!(blocks, Some(67));
    }
    ...
    fn test_parse_par2_case_insensitive() {
        ...
        let (set3, vol3, blocks3) = parse_par2_filename("MyFile.Vol63-67.PAR2");
        assert_eq!(set3, Some("MyFile".into()));
        assert_eq!(vol3, Some(63));
        assert_eq!(blocks3, Some(67));
    }
